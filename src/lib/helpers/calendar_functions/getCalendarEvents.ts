 ...
 */
export async function getManagedUsersByClientId(
  clientId: number
...
export async function fetchCalBookingsForUser(
  ...
  try {
    ...
    if (options.eventTypeIds)
      queryParams.append("eventTypeIds", options.eventTypeIds);
    if (options.afterStart)
      queryParams.append("afterStart", options.afterStart);
    ...
    if (result.status === "success") {
      ...
      console.log("result.error instanceof Error", result.error);
      console.log(
      ...
      if (result.error && isTokenExpiredError(response, result.error.message)) {
        ...
        if (newTokens) {
          ...
          if (updatedUser) {
            ...
            return await fetchCalBookingsForUser(updatedUser as CalManagedUser);
          }
        }
    ...
    console.error(
      `Error fetching Cal.com bookings for user ${managedUser.email}:`,
      error
    );
    return [];
  }
...
export async function getCalendarEvents(
  ...
  try {
    ...
    if (!hasConnectedCalendars) {
      console.log(
        `❌ Client ${clientId} has no connected calendars. Cannot fetch events.`
      );
      return [];
    }
    ...
    if (!hasEventTypes) {
      console.log(
        `❌ Client ${clientId} has no active event types. Cannot fetch events.`
      );
      return [];
    }
    ...
    if (!eventTypeIds) {
      ...
      if (!eventTypeIds) {
        console.log(
          `❌ Could not retrieve event type IDs for client ${clientId}`
        );
        return [];
      }
    ...
    console.log(
      `👥 Found ${managedUsers.length} managed users. Fetching bookings with event type IDs...`
    );
    ...
    console.log(
      `📅 Successfully fetched ${allBookings.length} calendar events for client ${clientId}`
    );
...
export async function searchBookings(
  ...
  try {
    console.log(
      `🔍 Searching bookings for client ${clientId} with criteria:`,
      searchCriteria
    );
    ...
    console.log(
      `📋 Found ${allBookings.length} bookings in date range, applying filters...`
    );
    ...
    if (searchCriteria.title) {
      ...
      console.log(
        `🏷️ After title filter ("${searchCriteria.title}"): ${filteredBookings.length} bookings`
      );
    }
    ...
    if (searchCriteria.attendeeEmail) {
      ...
      console.log(
        `📧 After attendee filter ("${searchCriteria.attendeeEmail}"): ${filteredBookings.length} bookings`
      );
    }

    console.log(
      `✅ Final search result: ${filteredBookings.length} matching bookings`
    );
...
export async function findBookingForReschedule(
  ...
  try {
    console.log(
      `🎯 Finding booking for reschedule: "${title}" on ${date || "any date"}`
    );
    ...
    if (date) {
      ...
      if (date.toLowerCase().includes("today")) {
        ...
        try {
          ...
          console.log(
            `⚠️ Could not parse date "${date}", searching all dates Error: ${error}`
          );
        }
      }
    }
    ...
    if (matchingBookings.length === 1) {
      console.log(
        `✅ Found exactly one matching booking: ${matchingBookings[0].uid}`
      );
    ...
    if (upcomingBookings.length > 0) {
      console.log(
        `🔄 Multiple matches found, returning closest upcoming booking: ${upcomingBookings[0].uid}`
      );
    ...
    console.log(
      `📅 No upcoming matches, returning most recent booking: ${sortedBookings[0].uid}`
    );
